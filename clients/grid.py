...
#region helpers     
    ...
    def ssapi_formatted_datetime(self):
        '''for posting to a date column smartsheet api'''
        # # Define the PST timezone
        pst_zone = pytz.timezone('America/Los_Angeles')

        # Get the current time directly in PST
        current_date_time = datetime.now(pst_zone)

        # Format the datetime object to the ISO 8601 format
        formatted_date_time = current_date_time.strftime('%Y-%m-%dT%H:%M:%S')       

        return formatted_date_time
    def verbose_formatted_datetime(self):
        '''for leaving error messages'''
        # Create a timezone object using pytz
        timezone = pytz.timezone("America/Los_Angeles")

        # Get the current date and time in UTC, then convert it to the specified timezone
        current_time = datetime.now(pytz.utc).astimezone(timezone)

        # Format the date and time in the desired format: 'DD/MM HH:MM AM/PM TIMEZONE'
        formatted_time = current_time.strftime('%m/%d %I:%M %p %Z')
